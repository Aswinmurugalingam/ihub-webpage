...
export function FaqSection(){
  ...
  return <section className={styles.altSec}><div className="container"><ScrollReveal><div className="section-header"><span className="tag">FAQ</span><h2 style={{marginTop:14}}>Before you <span className="brand-text">book</span></h2><p>Clear answers about estimates, parts, warranty, data and doorstep service.</p></div></ScrollReveal><div className={styles.faqList}>{FAQ_ITEMS.map((item,i)=>{const expanded=open===i;return <ScrollReveal key={i} delay={i*.035}><div className={`${styles.faqItem} ${expanded?styles.faqOpen:''}`}><button type="button" className={styles.faqQ} onClick={()=>setOpen(expanded?null:i)} aria-expanded={expanded} aria-controls={`faq-answer-${i}`}><span>{item.q}</span><span className={styles.faqChevron} aria-hidden="true">{expanded?'−':'+'}</span></button><div id={`faq-answer-${i}`} className={styles.faqAnswerWrap} aria-hidden={!expanded}><div className={styles.faqAnswerClip}><div className={styles.faqA}>{item.a}</div></div></div></div></ScrollReveal>})}</div></div></section>;
}
