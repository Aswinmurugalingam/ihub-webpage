...
export default function About() {
  ...
  return <>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div className={styles.heroGrid}>
          <motion.div className={styles.heroCopy} initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }}>
            ...
            <div className={styles.heroActions}><button onClick={() => navigate('/booking')} className="btn-primary brand-grad">Book Repair</button><button onClick={() => navigate('/pickup-delivery')} className="btn-secondary">Free Pickup & Delivery</button></div>
          </motion.div>
          <RepairCore3D />
        </div>
      </div>
    ...
    <section className={styles.ctaSec}><div className="container"><CtaBanner title={<>Start with a clear <span className="brand-text">diagnosis.</span></>} sub="Book a store appointment or request free pickup & delivery for an eligible mobile-phone repair." btn1Text="Schedule Free Pickup" btn1Link="/booking?method=pickup" btn2Text="Book Repair" btn2Link="/booking" /></div></section>
  </>;
}
